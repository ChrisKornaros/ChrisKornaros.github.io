...
3. [Linux Server Basics](@sec-linux_basics)
...
4. [Networking Basics](@sec-network)

   - [Computer Networking Basics](@sec-network_basics)
...
5. [SSH (Secure Shell)](@sec-ssh)

   - [SSH Basics](@sec-ssh_basics)
     - Client vs server setup
     - Key-based authentication
   - [Key-Based Authentication](@sec-ssh_keys)
...
7. [Backups and Basic Automation](@sec-backups)
   
   - [Backup Basics](@sec-backups_basics)
     - Directory structure and permissions
   - [Configuration Files Backup](@sec-backups_configs)
...
8. [Changing Your Boot Media Device](@sec-change_boot_media)

   - [Boot Basics](@sec-boot_basics)
     - Basics of transitioning boot media
     - Key concepts
   - [Boot Configuration Transition](@sec-boot_transition)
...
9. [Remote Development with VS Code](@sec-remote_with_vs_code)

   - Setting Up VS Code Remote SSH
   - Managing Remote Projects
   - Debugging and Terminal Integration

10. [Monitoring and Maintenance](@sec-monitor_maintain)

    - [Monitoring Basics](@sec-monitor_basics) 
      - `smartmon`
      - `vcgencmd`
    - [Log Management](@sec-monitor_logs)
      - Log basics
      - Management tools and strategy
    - [Performance Optimization](@sec-monitor_optimize)
      - Identifying areas to optimize
      - Optimization strategies
    - [Security Updates and Patching](@sec-monitor_patching)
      - 

11. [Containerization with Docker](@sec-docker)

    - [Containerization and Virtualization Basics](@sec-docker_basics)
    - [Docker Installation and Setup](@sec-docker_setup)
    - [Creating Images with Dockerfile](@sec-docker_images)
    - [Managing Images and Containers](@sec-docker_manage)
    - [Docker Compose for Multi-container Applications](@sec-docker_compose)
    - [CI/CD Integration](@sec-docker_ci_cd)

12. [Container Orchestration with Kubernetes](@sec-kubernetes)

    - [Orchestration Basics](@sec-kubernetes_basics)
    - [Kubernetes Concepts, Installation, and Setup](@sec-kubernetes_setup)
    - [Setting Up a Kubernetes Cluster](@sec-kubernetes_cluster)
    - [Deployment Strategies](@sec-kubernetes_deployment)
    - [Managing Resources](@sec-kubernetes_resources)
    - [Scaling Applications](@sec-kubernetes_scaling)
...
- **X11 Forwarding:** A mechanism for securely running graphical applications over a network.
...
- **Bastion Host:** A special-purpose server designed to withstand attacks, situated outside the firewall.
...
### Final Thoughts
At the time of writing this, I've tested and revised everything up until this point. There are a few things I haven't added yet, that I want to later, once it's necessary. 

1. Saving the installed packages and their requirements, similar to a `uv` lockfile or `.venv` requirements.txt file.
   1. Modify the restore script to restore packages to exactly where they should be
   2. Not necessary now, because only Traceroute, ufw, and fail2ban were installed; however, when other apps are added this will be helpful for tracking dependencies and versioning issues
2. Creating a private repoistory for storing configuration files, scripts, etc. and then using that, rather than rsync with my laptop
3. Do some more granular network configurations, inlcuding subnets and static IPs
...
### Boot Media Basics {#sec-boot_basics}
