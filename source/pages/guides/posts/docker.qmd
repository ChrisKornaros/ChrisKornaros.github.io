...
Before writing your first Dockerfile, it's worth setting up version control so that every configuration change is tracked and backed up remotely. This section walks through installing `git` and the GitHub CLI (`gh`) on your server, authenticating, creating a repository, and cloning it to your working directory. To keep things short, I will assume you are familiar with git and not go into the basic commands.

**Installing Git**

Ubuntu Server typically ships with `git` pre-installed. Verify with:

```bash
git --version
```

If you get `git: command not found`, install it:
...
Then configure your identity. Git attaches this information to every commit, and GitHub uses the email to link commits to your account:
...
- `user.email` must match an email on your GitHub account, otherwise commits appear as "unverified" and won't link to your profile.
- `init.defaultBranch main` ensures new repositories use `main` instead of the legacy `master`, matching GitHub's default.
- `--global` writes to `~/.gitconfig` and applies to all repositories for your user. You can override these per-repository by running the same commands without `--global` from inside a specific project.

**Installing the GitHub CLI**

When you're ready, simply run the following commands to install the program and validate its success.
...
- Select **SSH** as your protocol. You'll generate a dedicated key for GitHub in the next step, and SSH avoids storing tokens in plaintext for every push and pull.
- Select **No** when asked to generate a new key. The CLI's auto-generated key won't have a descriptive name or match your existing key naming conventions. We'll create one manually with proper naming.
- Select **Login with a web browser**. On a headless server, the CLI will display a one-time code and URL. Open the URL on your client machine (your MacBook), enter the code, and authorize the session. Your GitHub password and 2FA are handled entirely in the browser — nothing sensitive passes through the server's terminal.

**Generating an SSH Key for GitHub**

Your server needs its own SSH key pair to authenticate outward to GitHub. The SSH keys from your server guide (e.g., `ubuntu_pi_ecdsa`) live on your MacBook and authenticate *into* the Pi — the Pi itself doesn't have a private key for outbound connections. Generate a dedicated key:

```bash
ssh-keygen -t ecdsa -b 384 -C "ubuntu-pi-server-github" -f ~/.ssh/github_ecdsa
```

- `-f ~/.ssh/github_ecdsa` names the key explicitly so it doesn't collide with any defaults and its purpose is immediately clear.
- `-C "ubuntu-pi-server-github"` sets a descriptive comment to distinguish it from your MacBook's keys in any key listing.
- The command will prompt for a passphrase. On a server already secured behind SSH key auth, UFW, and Fail2Ban, leaving it empty is reasonable for convenience. Set one if you prefer an extra layer.

Upload the public key to your GitHub account using the CLI:

```bash
gh ssh-key add ~/.ssh/github_ecdsa.pub --title "ubuntu-pi-server"
```

If this fails with an `HTTP 404` error about the `admin:public_key` scope, the default OAuth token doesn't include permission to manage SSH keys. Refresh the token with the required scope:

```bash
gh auth refresh -h github.com -s admin:public_key
```

This triggers another browser authorization flow. The approval screen will explicitly list **Public keys** as a requested permission. After approving, re-run the upload command. GitHub's OAuth scopes are granular by design — SSH key management is treated as a higher-privilege operation because adding a key grants push access to all repositories your account can write to.

Then configure SSH on the server to use this key when connecting to GitHub:
...
- `User git` is required because all GitHub SSH connections authenticate as the user `git`. Your actual identity is determined by which key you present.
- `IdentitiesOnly yes` prevents SSH from offering other keys from your agent or default locations. Without this, SSH might exhaust its authentication attempts on wrong keys before trying `github_ecdsa`.
- `chmod 600` restricts the config file to owner-only read/write. SSH refuses to use config files readable by other users.
...
You should see `Hi YourUsername! You've successfully authenticated, but GitHub does not provide shell access.` The "no shell access" message is normal — GitHub's SSH endpoint only supports git operations.

**Creating and Cloning the Repository**

With authentication working, create your project repository:

```bash
gh repo create your-repo-name --public --readme --gitignore Python --license "AGPL-3.0" --clone
```

- `--public` or `--private` sets repository visibility. Use `--private` for projects with environment-specific configurations or sensitive files.
- `--readme` initializes the repository with a `README.md`, which ensures the `main` branch exists immediately.
- `--gitignore Python` adds a standard Python `.gitignore` template. You can customize it further after cloning.
- `--license "AGPL-3.0"` adds a license file. Substitute with your preferred license.
- `--clone` clones the repository to a local directory after creation. This is the step that previously failed before SSH was configured, so make sure `ssh -T git@github.com` succeeds first.

After cloning, navigate into the repository and verify:
...
The remote URLs should show `git@github.com:YourUsername/your-repo-name.git`, confirming SSH protocol is in use. From here, the standard workflow for saving your progress as you build out Dockerfiles and Compose configurations is:
