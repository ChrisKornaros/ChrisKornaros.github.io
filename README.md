# Docker on Ubuntu Server ARM64: installation, security, and firewall guide
...
## Installing Docker Engine from the official apt repository
...
### Solution 1: `"iptables": false` in daemon.json (not recommended)

Setting `{"iptables": false}` in `/etc/docker/daemon.json` prevents Docker from creating any iptables rules. This stops the UFW bypass but **breaks container networking entirely**: containers lose internet access (no masquerade/SNAT rules), inter-container communication across bridge networks fails, and you must manually write iptables rules for every Docker network. Docker's own documentation warns: *"This option is not appropriate for most users."*

### Solution 2: the DOCKER-USER chain (direct iptables approach)

Docker processes rules in the `DOCKER-USER` chain before its own chains, and guarantees it will never modify rules placed there. This is the officially sanctioned interception point.

**Important detail**: by the time packets reach DOCKER-USER, DNAT has already occurred. To match on original destination ports, use the `conntrack` extension with `--ctorigdstport` and `--ctdir ORIGINAL`.

Example: block all external access to containers, then selectively allow:

```bash
# Allow established connections (must come first)
sudo iptables -I DOCKER-USER -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT

# Allow internal Docker bridge traffic
sudo iptables -I DOCKER-USER -i docker0 -o docker0 -j ACCEPT

# Allow a specific IP to reach container port 8080
sudo iptables -A DOCKER-USER -i eth0 -s 203.0.113.50 -p tcp \
  -m conntrack --ctorigdstport 8080 --ctdir ORIGINAL -j ACCEPT

# Drop everything else from the external interface
sudo iptables -A DOCKER-USER -i eth0 -j DROP
```

These rules disappear on reboot. To persist them, create a systemd service that runs after Docker:

```ini
# /etc/systemd/system/docker-user-rules.service
[Unit]
Description=Custom DOCKER-USER iptables rules
...
ExecStart=/sbin/iptables -A DOCKER-USER -i eth0 -s 203.0.113.50 -p tcp -m conntrack --ctorigdstport 8080 --ctdir ORIGINAL -j ACCEPT
ExecStart=/sbin/iptables -A DOCKER-USER -i eth0 -j DROP
RemainAfterExit=yes
...
Enable with `sudo systemctl daemon-reload && sudo systemctl enable docker-user-rules.service`.

### Solution 3: the `/etc/ufw/after.rules` approach (recommended for UFW-managed servers)

This is the most elegant solution for servers already using UFW. Popularized by the [chaifeng/ufw-docker](https://github.com/chaifeng/ufw-docker) project (6.3k GitHub stars, updated November 2025), it redirects Docker traffic into UFW's forwarding chain so that UFW becomes the control point for container access.
...
Restart UFW with `sudo systemctl restart ufw` or `sudo ufw reload`. If rules don't take immediate effect, reboot.

After applying these rules, **all published Docker ports are blocked by default** from external access. Container-to-container communication and traffic from private subnets (LAN) are allowed. To selectively open a container port to external traffic, use `ufw route allow` (not `ufw allow`, which only controls the INPUT chain):
